...
public final class HologramFluidSimulator {
    private static final List<FluidProps> FLUID_REGISTRY = new ArrayList<>();
    ...
    public static void registerFluid(byte sourceId, byte flowingId, int dropOff, int maxDistance) {
        FLUID_REGISTRY.add(new FluidProps(sourceId, flowingId, dropOff, maxDistance));
    }

    public static void simulate() {
        for (FluidProps props : FLUID_REGISTRY) {
            simulateFluid(props);
        }
    }

    private static void simulateFluid(FluidProps props) {
    ...
    private static boolean hasAirNeighbor(int x, int y, int z) {
        if (y > 0 && AlphaByteCache.getBlockSafely(x, y - 1, z) == 0) return true;
        for (int[] d : HORIZONTAL) {
            if (AlphaByteCache.getBlockSafely(x + d[0], y, z + d[1]) == 0) return true;
        }
        return false;
    }

    private static void spreadFrom(int sx, int sy, int sz, FluidProps props, Set<Long> visited) {
    ...
    static {
        FLUID_REGISTRY.add(new FluidProps((byte) 9, (byte) 8, 1, 7));
        FLUID_REGISTRY.add(new FluidProps((byte) 11, (byte) 10, 2, 3));
    }

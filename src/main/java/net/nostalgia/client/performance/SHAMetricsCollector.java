...
public class SHAMetricsCollector {
  ...
  public static synchronized void recordCompile(long durationNs) {
    ...
    float ms = (float)durationNs / 1000000.0F;
    compileHistory.addLast(ms);
    if (compileHistory.size() > 60) {
  ...
  public static synchronized void recordDiskWrite(String target, long durationNs, long bytes) {
    ...
    if (diskWriteHistory.size() > 60) {
      diskWriteHistory.removeFirst();
    }

    logEvent(String.format(Locale.US, "Disk write: %s (%.2f MB in %.1f ms, %.1f MB/s)", target, mb, ms, ms > 0.0F ? mb / (ms / 1000.0) : 0.0));
  }
  ...
  public static synchronized void recordDiskRead(String target, long durationNs, long bytes) {
    ...
    if (diskReadHistory.size() > 60) {
      diskReadHistory.removeFirst();
    }

    logEvent(String.format(Locale.US, "Disk read: %s (%.2f MB in %.1f ms, %.1f MB/s)", target, mb, ms, ms > 0.0F ? mb / (ms / 1000.0) : 0.0));
  }
  ...
  public static void drawHUD(GuiGraphics graphics, DeltaTracker tracker) {
    if (overlayActive) {
      ...
      if (!mc.options.hideGui && mc.screen == null) {
        ...
        int y = 10;
        graphics.fill(x, y, x + w, y + h, -837742314);
        graphics.fill(x, y, x + w, y + 1, 1090519039);
        graphics.fill(x, y + h - 1, x + w, y + h, 1073741824);
        graphics.fill(x, y, x + 1, y + h, 1090519039);
        graphics.fill(x + w - 1, y, x + w, y + h, 1073741824);
        int ty = y + 8;
        graphics.drawString(font, "\u00a7b\u00a7lSHA PERFORMANCE PROFILER", x + 12, ty, -16711681, false);
        ty += 12;
        graphics.fill(x + 10, ty, x + w - 10, ty + 1, 637534207);
        ty += 8;
        ...
        graphics.drawString(
          font,
          "\u00a7fSky Render: \u00a7a"
            + String.format(Locale.US, "%.3f ms", avgRender)
            + " \u00a77(last: "
            + String.format(Locale.US, "%.2f", (float)lastRenderTimeNs / 1000000.0F)
            + ")",
          x + 12,
          ty,
          -1,
        ...
        graphics.drawString(
          font,
          "\u00a7fSodium Spoof: \u00a7a"
            + String.format(Locale.US, "%.3f ms", avgCompile)
            + " \u00a77(last: "
            + String.format(Locale.US, "%.2f", (float)lastCompileTimeNs / 1000000.0F)
            + ")",
          x + 12,
          ty,
          -1,
        ...
        ty += 12;
        graphics.drawString(font, "\u00a7fRAM cache: \u00a7d" + String.format(Locale.US, "%.2f MB", totalRAM), x + 12, ty, -1, false);
        ty += 12;
        graphics.drawString(font, "\u00a7fCached chunks: \u00a7e" + alphaChunks + " \u00a77(Alpha)", x + 12, ty, -1, false);
        ty += 12;
        graphics.drawString(font, "\u00a7fCached sections: \u00a7e" + totalSections + " \u00a77(Other)", x + 12, ty, -1, false);
        ty += 12;
        graphics.drawString(font, "\u00a7fOverrides: \u00a7e" + totalOverrides + " \u00a77(Deltas)", x + 12, ty, -1, false);
        ty += 12;
        boolean renderActive = PortalSkyRenderer.active;
        boolean groundedActive = false;
        graphics.drawString(
          font, "\u00a7fHolograms: " + (renderActive ? "\u00a7aPORTAL" : (groundedActive ? "\u00a7aGROUNDED" : "\u00a7cOFF")), x + 12, ty, -1, false
        );
        ty += 12;
        ty += 4;
        graphics.fill(x + 10, ty, x + w - 10, ty + 1, 637534207);
        ty += 6;
        graphics.drawString(font, "\u00a7b\u00a7lDISK CACHE SYSTEM:", x + 12, ty, -16711681, false);
        ty += 12;
        String readText = lastDiskReadTimeMs > 0L
          ? String.format(Locale.US, "\u00a7a%d ms \u00a77(%.2f MB)", lastDiskReadTimeMs, lastDiskReadSizeMB)
          : "\u00a77None";
        graphics.drawString(font, "\u00a7fLast Load: " + readText, x + 12, ty, -1, false);
        ty += 12;
        String writeText = lastDiskWriteTimeMs > 0L
          ? String.format(Locale.US, "\u00a7a%d ms \u00a77(%.2f MB)", lastDiskWriteTimeMs, lastDiskWriteSizeMB)
          : "\u00a77None";
        graphics.drawString(font, "\u00a7fLast Save: " + writeText, x + 12, ty, -1, false);
        ty += 12;
        ty += 6;
        graphics.drawString(font, "\u00a77Latency Timeline (last 50 frames)", x + 12, ty, -5592406, false);
        ty += 12;
        int graphH = 45;
        int graphY = ty;
        graphics.fill(x + 10, ty, x + w - 10, ty + graphH, 369098751);
        graphics.fill(x + 10, ty, x + w - 10, ty + 1, 822083583);
        graphics.fill(x + 10, ty + graphH - 1, x + w - 10, ty + graphH, 822083583);
        synchronized (SHAMetricsCollector.class) {
          ...
          for (int i = 0; i < barCount; i++) {
            ...
            int barH = (int)Math.min((float)graphH, ms / 16.67F * graphH);
            if (barH < 1) {
              barH = 1;
            }
        ...
        graphics.fill(x + 10, ty, x + w - 10, ty + 1, 637534207);
        ty += 6;
        graphics.drawString(font, "\u00a7b\u00a7lLAST HOLOGRAM EVENT:", x + 12, ty, -16711681, false);
        ty += 12;
        ...
        graphics.drawString(font, "\u00a7e" + lastEvent, x + 12, ty, -1, false);
      }
    }
  }
  ...
  public static String exportReport() {
    try {
      ...
      writer.write(
        "- **Last Disk Load Time**: "
          + (
            lastDiskReadTimeMs > 0L
              ? String.format(Locale.US, "%d ms (Size: %.2f MB, Target: %s)", lastDiskReadTimeMs, lastDiskReadSizeMB, lastDiskReadTarget)
              : "N/A"
          )
          + "\n"
      );
      writer.write(
        "- **Last Disk Save Time**: "
          + (
            lastDiskWriteTimeMs > 0L
              ? String.format(Locale.US, "%d ms (Size: %.2f MB, Target: %s)", lastDiskWriteTimeMs, lastDiskWriteSizeMB, lastDiskWriteTarget)
              : "N/A"
          )
          + "\n"
      );

...
public class HologramMiniRenderer {
  ...
  public void render(GuiGraphics graphics, int x, int y, int w, int h, float partialTick) {
    this.viewX = x;
    this.viewY = y;
    this.viewW = w;
    this.viewH = h;
    ...
    if (this.data != null && this.data.isReady()) {
      ...
      if (this.selectedLanding != null) {
        ...
        graphics.drawString(Minecraft.getInstance().font, coords, x + w - textW - 2, y + h - 9, -16720419, false);
      }
    }
  }

  private void renderModeToggle(GuiGraphics graphics, int x, int y, int w) {
    int btnX = x + w - 11;
    int btnY = y + 2;
    int btnW = 9;
    int btnH = 9;
  ...
  private void drawIsoDiamond(GuiGraphics graphics, int mx, int my, int dw, int dh, int color, boolean outlineOnly) {
    int halfW = dw / 2;
    int halfH = dh / 2;
    if (halfW >= 1 && halfH >= 1) {
  ...
  private void renderMarker(GuiGraphics graphics, int x, int y, int w, int h) {
    if (this.selectedLanding != null) {
      float[] projected = new float[2];
      int baseY = 64;

...
public final class CrtMonitorRenderer {
  public static void render(
    ...
    if (noSignalAlpha > 0.0F) {
      ...
      graphics.drawString(font, text, 1, 1, tShadowCol, false);
      graphics.drawString(font, text, 0, 0, tMainCol, false);
    ...
    if (bootTextAlpha > 0.0F) {
      ...
      if (timeSinceOpen < 150L) {
        if (drawCursor) {
          graphics.drawString(font, "_", ovX + 6, ty, mainCol, false);
        }
      } else if (timeSinceOpen < 350L) {
        graphics.drawString(font, "> BOOTING SYSTEM...", ovX + 6, ty, mainCol, false);
        if (drawCursor) {
          graphics.drawString(font, "_", ovX + 6, ty + 10, mainCol, false);
        }
      } else if (timeSinceOpen < 550L) {
        graphics.drawString(font, "> BOOTING SYSTEM...", ovX + 6, ty, mainCol, false);
        graphics.drawString(font, "> INIT MEMORY CACHE...", ovX + 6, ty + 10, mainCol, false);
        if (drawCursor) {
          graphics.drawString(font, "_", ovX + 6, ty + 20, mainCol, false);
        }
      } else if (timeSinceOpen < 750L) {
        graphics.drawString(font, "> BOOTING SYSTEM...", ovX + 6, ty, mainCol, false);
        graphics.drawString(font, "> INIT MEMORY CACHE...", ovX + 6, ty + 10, mainCol, false);
        graphics.drawString(font, "> LINK ESTABLISHED.", ovX + 6, ty + 20, mainCol, false);
        if (drawCursor) {
          graphics.drawString(font, "_", ovX + 6, ty + 30, mainCol, false);
        }
      } else if (timeSinceOpen < 950L) {
        graphics.drawString(font, "> BOOTING SYSTEM...", ovX + 6, ty, mainCol, false);
        graphics.drawString(font, "> INIT MEMORY CACHE...", ovX + 6, ty + 10, mainCol, false);
        graphics.drawString(font, "> LINK ESTABLISHED.", ovX + 6, ty + 20, mainCol, false);
        graphics.drawString(font, "> STATUS: READY", ovX + 6, ty + 30, mainCol, false);
        if (drawCursor) {
          graphics.drawString(font, "_", ovX + 6, ty + 40, mainCol, false);
        }
      } else {
  ...
  public static void renderBorderOnly(GuiGraphics graphics, int x, int y, boolean isOverloading, float overloadProgress) {
    int vX = x + 8;
    int vY = y + 14;
  ...
  public static void renderOverloadOverlay(GuiGraphics graphics, int x, int y, float gameTime, float overloadProgress, int currentEnergyColor) {
    int vX = x + 8;
    int vY = y + 14;
  ...
  public static void renderCrtOverlay(GuiGraphics graphics, int x, int y, float gameTime) {
    renderCrtOverlay(graphics, x, y, gameTime, 1.0F);
  }

  public static void renderCrtOverlay(GuiGraphics graphics, int x, int y, float gameTime, float alpha) {
    if (!(alpha <= 0.0F)) {
      int vX = x + 8;
      int vY = y + 14;
      int mW = 160;
      ...
      graphics.renderOutline(vX, vY, mW, mH, vColor1);
      graphics.renderOutline(vX + 1, vY + 1, mW - 2, mH - 2, vColor2);
      graphics.renderOutline(vX + 2, vY + 2, mW - 4, mH - 4, vColor3);
  ...
  public static void renderGlitchOverlay(GuiGraphics graphics, int monX, int monY, float gameTime, float glitchAlpha) {
    Random rand = new Random((long)(gameTime * 13.0F));
    int numGlitches = rand.nextInt(7) + 5;

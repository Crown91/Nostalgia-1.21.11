...
public class EpilepsyWarningScreen extends Screen {
  ...
  public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
    ...
    graphics.drawCenteredString(this.font, boldTitle, this.width / 2, y + 25, -1);
    Component warningText = Component.translatable("gui.nostalgia.warning.text").copy().withStyle(Style.EMPTY.withBold(true));
    int textY = y + 108;
    graphics.drawWordWrap(this.font, warningText, this.width / 2 - 85, textY, 170, -1, true);
    int btnW = 130;
    int btnH = 30;
    int btnX = this.width / 2 - btnW / 2;
    int btnY = this.height / 2 + 45;
    ...
    graphics.drawCenteredString(this.font, continueText, this.width / 2, btnY + 11, btnColor);

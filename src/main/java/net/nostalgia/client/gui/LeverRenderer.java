...
public final class LeverRenderer {
    ...
    public static void render(
        ...
        int minLx = Math.min(cx, lx2), maxLx = Math.max(cx, lx2);
        int minLy = Math.min(cy, ly2), maxLy = Math.max(cy, ly2);
        graphics.fill(minLx, minLy, maxLx + 1, maxLy + 1, 0xFF888888);
        graphics.fill(minLx + 1, minLy, maxLx + 1, maxLy + 1, 0xFFBBBBBB);

        int headX = lx2 - 2, headY = ly2 - 2;
        int headColor = isOverloading ? currentEnergyColor : (renderedProgress >= 1.0f ? TimeMachineLayout.GOLD_PRIMARY : 0xFF2A1E10);

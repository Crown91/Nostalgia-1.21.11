...
public final class NixieLampRenderer {
  ...
  public static void render(
    GuiGraphics graphics, Font font, int lx, int ly, int w, int h, char ch, float brightness, int energyColor, boolean isHovered
  ) {
    initTextures();
    ...
    if (ch != ' ' && brightness > 0.0F) {
      ...
      graphics.drawString(font, sym, tx, ty, glowAlphaInner << 24 | glowColInner & 16777215, false);
      int coreAlpha = (int)(255.0F * brightness);
      int coreColor = -3936;
      graphics.drawString(font, sym, tx, ty, coreAlpha << 24 | coreColor & 16777215, false);
    }
  }

...
public class AlphaBoatRenderer extends EntityRenderer<AlphaBoatEntity, BoatRenderState> {
  ...
  private final AlphaBoatModel model = new AlphaBoatModel(AlphaBoatModel.createBodyLayer().bakeRoot());

  public AlphaBoatRenderer(Context context) {
    super(context);
  ...
  public void submit(BoatRenderState state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState camera) {
    ...
    if (damageTime > 0.0F) {
      poseStack.mulPose(Axis.XP.rotationDegrees(Mth.sin(damageTime) * damageTime * state.hurtTime / 10.0F * state.hurtDir));
    }

...
public class AlphaChunkDecorator {
    ...
    public static void decorate(ChunkPos cp, long seed) {
        AlphaWorldGenTrees treeGen = new AlphaWorldGenTrees();
        AlphaWorldGenBigTreeHologram bigTreeGen = new AlphaWorldGenBigTreeHologram();
        AlphaWorldGenFlowers dandelionGen = new AlphaWorldGenFlowers((byte) 37);
        AlphaWorldGenFlowers poppyGen = new AlphaWorldGenFlowers((byte) 38);
        AlphaWorldGenFlowers brownMushroomGen = new AlphaWorldGenFlowers((byte) 39);
        AlphaWorldGenFlowers redMushroomGen = new AlphaWorldGenFlowers((byte) 40);

        if (treeNoise == null || lastSeed != seed) {
            synchronized (AlphaChunkDecorator.class) {
                if (treeNoise == null || lastSeed != seed) {
        ...
        for (int i = 0; i < treeCount; ++i) {
            ...
            int y = net.nostalgia.client.events.caches.impl.AlphaByteCache.getHighestBlockY(x, z);
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            for (int r = 0; r < 20; ++r) {
                ...
                if (dy >= 0 && dy < 128) {
                    byte blockId = net.nostalgia.client.events.caches.impl.AlphaByteCache.getBlockSafely(dx, dy, dz);
                    if (blockId == 0 || blockId == 83) {
                        byte belowState = net.nostalgia.client.events.caches.impl.AlphaByteCache.getBlockSafely(dx, dy - 1, dz);
                        if (belowState == 2 || belowState == 3 || belowState == 12) {
                            boolean adjacentWater = false;
                            int[][] dirs = {{1,0}, {-1,0}, {0,1}, {0,-1}};
                            for (int[] dir : dirs) {
                                byte adjBlock = net.nostalgia.client.events.caches.impl.AlphaByteCache.getBlockSafely(dx + dir[0], dy - 1, dz + dir[1]);
                                if (adjBlock == 8 || adjBlock == 9) {
                                    adjacentWater = true;
                                    break;
                                }
                            }
                            if (adjacentWater) {
                                ...
                                for (int h = 0; h < height; h++) {
                                    if (dy + h < 128 && net.nostalgia.client.events.caches.impl.AlphaByteCache.getBlockSafely(dx, dy + h, dz) == 0) {
                                        net.nostalgia.client.events.caches.impl.AlphaByteCache.setBlockSafely(dx, dy + h, dz, (byte) 83);
                                    }
                                }
                            }
                        }
        ...
        for (int i = 0; i < 1; ++i) {
            ...
            for (int k = 0; k < 10; ++k) {
                ...
                if (ty >= 0 && ty < 128 && net.nostalgia.client.events.caches.impl.AlphaByteCache.getBlockSafely(tx, ty, tz) == 0) {
                    int height = 1 + currentRand.nextInt(currentRand.nextInt(3) + 1);
                    for (int h = 0; h < height; ++h) {
                        byte below = net.nostalgia.client.events.caches.impl.AlphaByteCache.getBlockSafely(tx, ty + h - 1, tz);
                        if (below == 12 || below == 81) {
                            boolean isValid = true;
                            int[][] dirs = {{1,0}, {-1,0}, {0,1}, {0,-1}};
                            for (int[] dir : dirs) {
                                byte adjBlock = net.nostalgia.client.events.caches.impl.AlphaByteCache.getBlockSafely(tx + dir[0], ty + h, tz + dir[1]);
                                if (adjBlock != 0 && adjBlock != 8 && adjBlock != 9 && adjBlock != 10 && adjBlock != 11 && adjBlock != 18 && adjBlock != 20 && adjBlock != 83) {
                                    isValid = false; break;
                                }
                            }
                            if (isValid) {
                                net.nostalgia.client.events.caches.impl.AlphaByteCache.setBlockSafely(tx, ty + h, tz, (byte) 81);
                            }
                        }
                    }
                }
        ...
        if (isSnowMode) {
            for (int x = startX + 8; x < startX + 8 + 16; ++x) {
                for (int z = startZ + 8; z < startZ + 8 + 16; ++z) {
                    int y = net.nostalgia.client.events.caches.impl.AlphaByteCache.getHighestBlockY(x, z);
                    if (y > 0 && y < 128) {
                        byte blockId = net.nostalgia.client.events.caches.impl.AlphaByteCache.getBlockSafely(x, y, z);
                        if (blockId == 0) {
                            byte belowBlock = net.nostalgia.client.events.caches.impl.AlphaByteCache.getBlockSafely(x, y - 1, z);
                            if (belowBlock != 0 && belowBlock != 8 && belowBlock != 9 && belowBlock != 10 && belowBlock != 11 && belowBlock != 79 && belowBlock != 18 && belowBlock != 20 && belowBlock != 83) {
                                net.nostalgia.client.events.caches.impl.AlphaByteCache.setBlockSafely(x, y, z, (byte) 78);
                            }
                        }
                    }
                }

...
public class AlphaChunkDecorator {
  ...
  public static void decorate(ChunkPos cp, long seed) {
    ...
    for (int i = 0; i < 8; i++) {
      new AlphaWorldGenDungeonsHologram()
        .generate(currentRand, startX + currentRand.nextInt(16) + 8, currentRand.nextInt(128), startZ + currentRand.nextInt(16) + 8);
    }
    ...
    for (int i = 0; i < 20; i++) {
      skipOre(currentRand, 128, 16);
    }

    for (int i = 0; i < 20; i++) {
      skipOre(currentRand, 64, 8);
    }

    for (int i = 0; i < 2; i++) {
      skipOre(currentRand, 32, 8);
    }

    for (int i = 0; i < 8; i++) {
      skipOre(currentRand, 16, 7);
    }
    ...
    double d2 = 0.5;
    int treeCount = (int)((treeNoise.generateNoise((double)startX * d2, (double)startZ * d2) / 8.0 + currentRand.nextDouble() * 4.0 + 4.0) / 3.0);
    if (treeCount < 0) {
      treeCount = 0;
    }
    ...
    boolean isBigTree = currentRand.nextInt(10) == 0;
    ...
    for (int ix = 0; ix < 10; ix++) {
      ...
      for (int r = 0; r < 20; r++) {
        ...
        int dy = ry;
        if (dy >= 0 && dy < 128) {
          byte blockId = AlphaByteCache.getBlockSafely(dx, dy, dz);
          if (blockId == 0 || blockId == 83) {
            byte belowState = AlphaByteCache.getBlockSafely(dx, dy - 1, dz);
            if (belowState == 2 || belowState == 3) {
              boolean adjacentWater = false;
              int[][] dirs = new int[][]{{1, 0}, {-1, 0}, {0, 1}, {0, -1}};

              for (int[] dir : dirs) {
                byte adjBlock = AlphaByteCache.getBlockSafely(dx + dir[0], dy - 1, dz + dir[1]);
              ...
              if (adjacentWater) {
                ...
                for (int h = 0; h < height; h++) {
                  if (dy + h < 128 && AlphaByteCache.getBlockSafely(dx, dy + h, dz) == 0) {
                    AlphaByteCache.setBlockSafely(dx, dy + h, dz, (byte)83);
                  }
                }
              }
            }
    ...
    for (int ix = 0; ix < 1; ix++) {
      ...
      for (int k = 0; k < 10; k++) {
        ...
        if (ty >= 0 && ty < 128 && AlphaByteCache.getBlockSafely(tx, ty, tz) == 0) {
          int height = 1 + currentRand.nextInt(currentRand.nextInt(3) + 1);

          for (int hx = 0; hx < height; hx++) {
            byte below = AlphaByteCache.getBlockSafely(tx, ty + hx - 1, tz);
            if (below == 12 || below == 81) {
              boolean isValid = true;
              int[][] dirs = new int[][]{{1, 0}, {-1, 0}, {0, 1}, {0, -1}};

              for (int[] dirx : dirs) {
                byte adjBlock = AlphaByteCache.getBlockSafely(tx + dirx[0], ty + hx, tz + dirx[1]);
              ...
              if (isValid) {
                AlphaByteCache.setBlockSafely(tx, ty + hx, tz, (byte)81);
              }
            }
          }
        }
